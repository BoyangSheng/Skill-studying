...
​	上面的例子中，定义的子程序 add(n1, n2 ,n3) 在主程序中调用了一次，调用的时候，输入的实参是 a, b, c 三个变量所代表的值；而形参是定义子程序时使用的，在子程序内部用形参表示这个函数的功能。实参将值引入子程序，子程序按照形参在子程序中的定义将实参传入的值进行操作，最终输出子程序的返回值，返回值是 return 后面的变量所代表的数值，而这个数值在主程序中，是用子程序名所代替的，在上面的例子中，result 是子程序中返回的值得变量名，在子程序中，返回值的变量名都是 result；而在主程序中，result 所代表的值需要用 add(a, b, c) 来表示。简单来说，result 和 add(a, b, c) 的值是一样的，只不过 result 用在子程序中，add(a, b, c) 用在主程序中。

#### 4.2.2  函数和过程

​	函数，是指运行会有返回结果命令；过程，是不会直接返回一个结果的
